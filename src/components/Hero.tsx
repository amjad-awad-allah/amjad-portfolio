...
const Hero = () => {
  ...
  const skillLabels = [{
    ...
    rotate: 0,
    delay: 0.3,
    position: "right-0 translate-x-10"
  }, {
    text: "Full Stack Developer",
    rotate: 0,
    delay: 0.4,
    position: "bottom-0 translate-y-8"
  }];
  return <section id="home" className="py-24 md:py-32 relative overflow-hidden bg-[#080f16]">
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        ...
        {codeElements.map((item, index) => <motion.div key={index} className="absolute font-mono text-blue-500/20 dark:text-blue-400/15 pointer-events-none" style={{
        top: item.top,
        left: item.left,
        fontSize: item.size
      }} initial={{
        ...
        {/* Tech icons with pulse animation */}
        {techIcons.map((icon, index) => <div key={`tech-icon-${index}`} className="absolute pointer-events-none" style={{
        top: icon.top || "auto",
        ...
        left: icon.left || "auto"
          ...
          delay: index * 1.5
              ...
              <Cpu className="text-blue-500/30 dark:text-blue-400/30" size={icon.size} />
            </motion.div>
          </div>)}

        {/* Additional binary patterns */}
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
          {[...Array(8)].map((_, i) => <motion.div key={`binary-${i}`} className="absolute font-mono text-blue-500/10 dark:text-blue-400/15 text-xs" style={{
          top: `${Math.random() * 100}%`,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-16">
          ...
          duration: 0.5
            ...
            {isLoading ? <Skeleton className="rounded-full w-full h-full" /> : <>
                <img src={personalInfo?.profile_image_url || "/lovable-uploads/991c3f58-2d55-4c28-865f-e2e9eadb9c1e.png"} alt={personalInfo?.name || "Profile"} className="w-full h-full object-cover rounded-full border-4 border-gray-800" />
              ...
              duration: 0.5
                ...
                <div className="absolute -right-4 top-1/2 -translate-y-1/2 flex flex-col gap-3">
                  {personalInfo?.github_url && <motion.a href={personalInfo.github_url} target="_blank" rel="noopener noreferrer" className="bg-gray-800/90 p-2.5 rounded-full text-gray-300 hover:text-white border border-gray-700 hover:border-gray-600 transition-colors" aria-label="GitHub Profile" whileHover={{
                scale: 1.1
              }} whileTap={{
                scale: 0.95
              }}>
                  ...
                  {personalInfo?.linkedin_url && <motion.a href={personalInfo.linkedin_url} target="_blank" rel="noopener noreferrer" className="bg-gray-800/90 p-2.5 rounded-full text-gray-300 hover:text-white border border-gray-700 hover:border-gray-600 transition-colors" aria-label="LinkedIn Profile" whileHover={{
                scale: 1.1
              }} whileTap={{
                scale: 0.95
              }}>
